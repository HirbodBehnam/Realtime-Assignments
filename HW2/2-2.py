...
class Task:
    def __init__(self, id: int, release: int, deadline: int, computation: int, predecessors: list[int]) -> None:
        self.id = id
...
def earliest_deadline(tasks: Iterator[Task], current_time: int) -> Union[Task, None]:
    remaining_tasks = list(filter(lambda task: task.computation_left != 0 and task.new_release_time <= current_time, tasks))
...
tasks: dict[int, Task] = {}
tasks[1] = Task(1, 2, 20, 2, [])
tasks[2] = Task(2, 3, 20, 3, [])
tasks[3] = Task(3, 8, 30, 4, [1])
tasks[4] = Task(4, 4, 30, 4, [1])
tasks[5] = Task(5, 5, 30, 5, [1, 2])
tasks[6] = Task(6, 7, 30, 2, [2])
tasks[7] = Task(7, 6, 40, 3, [3, 4])
tasks[8] = Task(8, 8, 40, 1, [4, 5])
tasks[9] = Task(9, 10, 40, 4, [5, 6])
tasks[10] = Task(10, 7, 50, 3, [7, 8])
tasks[11] = Task(11, 11, 50, 3, [9])
tasks[12] = Task(12, 9, 50, 4, [9])
# Fix the release time of fathers
for task in tasks.values():
    if len(task.predecessors) == 0:
        print("Father", task.id)
        task.new_release_time = task.release
...
while any(map(lambda t: t.new_release_time == -1, tasks.values())):
    for current_task in tasks.values():
        if current_task.new_release_time == -1 and all(map(lambda id: tasks[id].new_release_time != -1, current_task.predecessors)):
            current_task.new_release_time = max(current_task.release, max(map(lambda id: tasks[id].new_release_time + tasks[id].computation, current_task.predecessors)))
...
for t in range(1000):
    to_schedule_task = earliest_deadline(tasks.values(), t)
    ...
    if to_schedule_task.computation_left == 0:
        print(f"Scheduling task {to_schedule_task.id} finished in T={t}")
